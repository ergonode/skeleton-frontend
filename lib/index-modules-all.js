...
${typeModules('npm').length ? `\t\tnpm: [\n\t\t\t\t${moduleStr(typeModules('npm'))}\n\t\t],` : ''}
    ...
    const moduleInstall = array => array.reduce((acc, module) => `${acc}${module.name || module}@${major}.${minor}.x `, '');
    ...
    fs.writeFile('./modules.config.js', fileStr, (err) => {
        ...
        console.log(chalk.yellow(`Installing npm modules in ${major}.${minor}.x version`));
